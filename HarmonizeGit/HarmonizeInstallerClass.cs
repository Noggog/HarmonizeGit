...
namespace HarmonizeGit.CustomSetup
{
    ...
    public class HarmonizeInstallerClass : Installer
    {
        ...
        {
            get => _targetDir ?? GetContextParametersTargetDir();
            set => _targetDir = value;
        }

        private string GetContextParametersTargetDir()
        {
            var targetDirParam = this.Context.Parameters["targetdir"];
            if (targetDirParam == null)
            {
                throw new ArgumentException("Context parameters null for key 'targetdir'.");
            }
            return targetDirParam.TrimEnd('\\') + '\\';
        }
        ...
        public bool GetMassHookDir(out DirectoryInfo dir)
        {
            ...
            dir = new DirectoryInfo(Path.Combine(fishingPath, FishingWithGit.Common.Constants.MASS_HOOK_FOLDER_NAME, "HarmonizeGit\\"));
            _MassHookDir = dir;
            return true;
        }
